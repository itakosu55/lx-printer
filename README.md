...
- **Raw Data Printing**: Send pre-formatted 384px binary data (`isRaw` mode).
...
// ... fill data ...
await printer.print(rawData, { isRaw: true });
```
...
#### `print(data: HTMLImageElement | HTMLCanvasElement | Uint8Array, options?: ImageProcessingOptions): Promise<void>`

Sends print data to the printer.

- `options.isRaw`: Set to `true` to skip internal dithering/resizing for `Uint8Array` inputs.
